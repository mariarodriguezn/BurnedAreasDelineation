...
class BurnedAreasDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            transformed = self.transform(image=data_array, mask=label_array, cloud_mask=cm_array)
            data_tensor = transformed["image"]
            label_tensor = transformed["mask"].unsqueeze(0)
            cm_tensor = transformed["cloud_mask"].unsqueeze(0)
        ...
        return data_tensor, label_tensor, patch_id, cm_tensor, dm_tensor
    ...
    def _get_transform(self, phase, filtered_means, filtered_stds):
        if phase == 'train':
            return A.Compose([
                ...
                ToTensorV2()
            ], additional_targets={'cloud_mask': 'mask'}) 
        else:
            return A.Compose([
                A.Normalize(mean=filtered_means, std=filtered_stds, max_pixel_value=1.0),
                ToTensorV2()
